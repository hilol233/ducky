<template>
  <div class="p-funds-item" :class="{ 'd-none': displayNone }">
    <div class="sec-1">
      <div class="money">R$ {{ fundItem.money }}</div>
      <div class="image">
        <img :src="fundItem.icon_url" alt="money icon" />
      </div>
    </div>
    <div class="sec-2">
      <div class="price">${{ fundItem.price }}</div>
      <button @click="purchase">Buy Now</button>
    </div>
  </div>
</template>

<script>
export default {
  props: ["fundItem", "fundIndx"],
  computed: {
    displayNone() {
      if (this.fundIndx > 1) {
        return true;
      } else {
        return false;
      }
    },
  },
  mounted() {
    //
  },
  methods: {
    getImgUrl(_asset) {
      return require(_asset);
    },
    purchase() {
      this.$store.commit("modals/SET_FIND_ACC_MODAL_DATA", this.fundItem);
      this.$store.commit("modals/OPEN_FEATURED_ITEM_MODAL");
      this.$store.commit("modals/OPEN_FIND_ACC_MODAL");
    },
  },
};
</script>

<style lang='scss' scoped>
@import "~assets/scss/variables";
@import "~assets/scss/mixins";
/**
@ PC First Design
 */

.d-none {
  @include break-down(small) {
    display: none;
  }
}

.p-funds-item {
  width: rem(200);
  height: rem(250);
  // margin-bottom: calc(rem(27) * 2);
  border-radius: rem(25);
  box-shadow: rgba(0, 0, 0, 0.16) 0px 3px 6px, rgba(0, 0, 0, 0.23) 0px 3px 6px;
  @include break-down(small) {
    width: rem(150);
    height: rem(200);
    border-radius: rem(15);
  }

  .sec-1 {
    height: rem(160);
    display: flex;
    flex-direction: column;
    align-items: center;
    padding-top: rem(15);
    background-color: $coal-dark;
    border-radius: rem(25) rem(25) 0 0;
    @include break-down(small) {
      height: rem(135);
      border-radius: rem(15) rem(15) 0 0;
    }

    .money {
      margin-bottom: rem(42);
      @include break-down(small) {
        margin-bottom: rem(30.47);
        font-size: rem(12);
        line-height: rem(14);
      }
    }
  }
  .sec-2 {
    display: flex;
    flex-direction: column;
    align-items: center;
    height: rem(90);
    padding-bottom: rem(11);
    background-color: $coal-light;
    border-radius: 0 0 rem(25) rem(25);
    @include break-down(small) {
      height: rem(65);
      border-radius: 0 0 rem(15) rem(15);
    }

    .price {
      margin-top: rem(11);
      @include break-down(small) {
        font-size: rem(14);
        line-height: rem(16);
      }
    }

    > button {
      width: rem(164);
      height: rem(32);
      font-size: rem(18);
      line-height: rem(21);
      color: #fff;
      margin-right: auto;
      margin-left: auto;

      background: #6d40cd;
      border: none;
      outline: none;
      cursor: pointer;
      margin-top: rem(10);
      box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);
      border-radius: rem(50);
      @include break-down(small) {
        width: rem(116.2);
        height: rem(21.05);
        font-size: rem(10);
        line-height: rem(12);
      }

      &:hover {
        color: #6d40cd;
        background-color: #fff;
        border: rem(2) solid #6d40cd;
      }
    }
  }
}
</style>